...
1. **QA 标准答案文件重命名**（backup/datasets/qa）
   - 将 root 与 backend 的 `merged_*.json` 统一更名为 `A1_full.json`/`A2_full.json`……`X_full.json`，并保留 trimmed 样例 `A1.json`…`X.json`
   - 同步更新文档与脚本引用（如 `backend/scripts/generate_test_predictions.py`）
...
1. **QA/VQA 样例 vs. 完整**
   - root/backend datasets 的每个 JSON 现在仅保留前 2 道题（作为公开示例），完整题库保存到 `backup/**/_full.json`（默认不入库）
   - 新增 `backend/scripts/trim_datasets.py`，可从 `backup/**/_full.json` 重新生成上述样例
   - `backend/scripts/generate_test_predictions.py` 读取 `backup/backend/datasets/**/_full.json`，确保冒烟预测仍覆盖全量题目
